...
2. **Script organization** - All scripts are properly copied to `.claude/scripts/supabase/`
...
|--------|--------|----------|
| supabase_api.py | ✅ | `.claude/scripts/supabase/` |
| save_project_credentials.py | ✅ | `.claude/scripts/supabase/` |
| create_project.py | ✅ | `.claude/scripts/supabase/` |
| list_projects.py | ✅ | `.claude/scripts/supabase/` |
| get_api_keys.py | ✅ | `.claude/scripts/supabase/` |
| execute_sql.py | ✅ | `.claude/scripts/supabase/` |
| list_tables.py | ✅ | `.claude/scripts/supabase/` |
| query_table.py | ✅ | `.claude/scripts/supabase/` |
| mutate_table.py | ✅ | `.claude/scripts/supabase/` |
| manage_rls.py | ✅ | `.claude/scripts/supabase/` |
| setup_auth.py | ✅ | `.claude/scripts/supabase/` |
...
|-------|--------|-------------------|----|
| supabase-project-manage | ✅ PASS | 12 references | `~/.claude/scripts/supabase/` |
| supabase-database | ✅ PASS | 19 references | `~/.claude/scripts/supabase/` |
| supabase-auth-setup | ✅ PASS | 7 references | `~/.claude/scripts/supabase/` |
| supabase-deployer | ✅ PASS | Uses local scripts | `~/.claude/scripts/supabase/` |
| No absolute paths | ✅ PASS | All relative paths | No `/root/` references |

**What this verifies:** All skill documentation uses consistent, portable paths to local scripts. Users can reference `~/.claude/scripts/supabase/` which expands to the correct location.
...
│   │   │   └── SKILL.md (references ~/.claude/scripts/supabase/*)
│   │   ├── supabase-database/
│   │   │   └── SKILL.md (references ~/.claude/scripts/supabase/*)
│   │   ├── supabase-auth-setup/
│   │   │   └── SKILL.md (references ~/.claude/scripts/supabase/*)
...
User executes skill
    ↓
Skill SKILL.md references: python3 ~/.claude/scripts/supabase/create_project.py
    ↓
...
1. **Proper Separation of Concerns**
   - Local scripts (`.claude/scripts/supabase/`) are project-specific
   - Global credentials (`/etc/supabase/`) are system-wide
   - Clean, maintainable architecture

2. **Consistent Path Usage**
   - All skills use `~/.claude/scripts/supabase/` (portable)
...
python3 ~/.claude/scripts/supabase/list_organizations.py

# Create a project and save credentials
python3 ~/.claude/scripts/supabase/create_project.py \
  --name "My MVP" \
  --region "eu-central-1" \
  --wait \
  --save-credentials
...
python3 ~/.claude/scripts/supabase/query_table.py \
  --ref "abcdefghij" \
  --table "posts" \
  --limit 10
```
...
python3 ~/.claude/scripts/supabase/save_project_credentials.py \
  --ref "abcdefghij" \
  --anon-key "your_key" \
...
```bash
ls -la ~/.claude/scripts/supabase/supabase_api.py
...
```bash
python3 ~/.claude/scripts/supabase/test-skills-credentials.py
```
...
python3 ~/.claude/scripts/supabase/list_projects.py

# Test 2: Check saved credentials
python3 ~/.claude/scripts/supabase/save_project_credentials.py --list
...
**Test Suite:** `.claude/scripts/supabase/test-skills-credentials.py`
