...
> For production, use `AGENTS_Reserve.md` (Kimi phases 0-4.2) or `CLAUDE_Reserve.md` (GLM-5 phases 4.3-7.1) instead.  
...
This meta-folder uses a **Kimi + GLM-5** hybrid architecture:
...
**Entry Points:**
- Kimi phases (0-4.2): `AGENTS.md` (meta-dev) / `AGENTS_Reserve.md` (production)
- GLM-5 phases (4.3-7.1): `CLAUDE.md` (meta-dev) / `CLAUDE_Reserve.md` (production)
...
Domain Skills (.shared/skills/)
    ↓
Agent Descriptions (.kimi/agents/ for Phases 0-4.2, .glm5/agents/ for Phases 4.3-7.1)
    ↓
Helper Scripts (.shared/scripts/)
```

**Key insight:** The slim process file is NOT self-contained. It links to detailed phase skills, which reference domain skills, which invoke agents (for GLM-5 Phase 1 work). To understand any part, you must trace through all connected files.
...
#### 2. Agent Descriptions (`.glm5/agents/<name>.md`)
...
COMPLEXITY: [low|medium|high|critical] — [Rationale]
...
3. Create file in appropriate location:
   - Phases 0-4.2 (Kimi): `.kimi/agents/<name>.md` with `model: kimi-k2.5`
   - Phases 4.3-7.1 (GLM-5): `.glm5/agents/<name>.md` with `model: GLM-5`
4. Write YAML frontmatter
5. Include MODEL indicator in instructions
6. Write detailed instructions
7. Update phase skill to reference this agent
...
- [ ] Complexity indicator specified
