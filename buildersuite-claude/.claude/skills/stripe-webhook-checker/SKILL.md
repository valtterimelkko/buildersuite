...
```bash
python3 .claude/scripts/stripe/check_webhooks.py --file "templates/TEMPLATE_NAME/frontend/app/api/stripe/webhook/route.ts"
```

### Check All Templates

```bash
python3 .claude/scripts/stripe/check_webhooks.py --all
```

### Check with Specific Billing Model

```bash
python3 .claude/scripts/stripe/check_webhooks.py --file "path/to/route.ts" --model "usage-based"
```
...
```bash
python3 .claude/scripts/stripe/check_webhooks.py \
