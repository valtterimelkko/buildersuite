...
(.claude/scripts/supabase/)   (/etc/supabase/)
        ↓                              ↑
...
python3 ~/.claude/scripts/supabase/get_api_keys.py --ref "abcdefghij"
...
python3 ~/.claude/scripts/supabase/setup_auth.py \
  --ref "abcdefghij" \
...
→ Verify `.claude/scripts/supabase/` directory exists
→ Verify all scripts are present: `ls ~/.claude/scripts/supabase/`
...
| `.claude/scripts/supabase/*` | Local scripts | 644 (readable) |
| `.claude/skills/supabase-*/*` | Skill documentation | 644 (readable) |
...
✅ **Do commit** `.claude/skills/` and `.claude/scripts/` - non-sensitive code
✅ **Portability** - All scripts use `~/.claude/scripts/supabase/` paths, works across systems
