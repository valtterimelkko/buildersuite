      ...
      <div className="flex-1 overflow-auto p-4">
        {projects && projects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map((project: { id: string; slug: string; name: string; description?: string; icon?: string; color: string; lead?: { display_name?: string }; issues?: Array<{ status: string }> }) => {
              const totalIssues = project.issues?.length || 0
              const doneIssues = project.issues?.filter((i: { status: string }) => i.status === 'done').length || 0
