...
export async function POST(request: Request) {
  ...
  try {
    switch (event.type) {
      ...
      default:
        // Handle meter error events (not in Stripe type definitions)
        if (event.type === ('billing.meter.error_report_triggered' as string)) {
          const meterEvent = event as any
          console.error('Stripe Meter error report:', JSON.stringify(meterEvent.data?.object, null, 2))
        } else {
