...
LOCAL_SCRIPTS = PROJECT_ROOT / ".claude" / "scripts" / "supabase"
...
def test_skill_documentation_paths(results):
    ...
    for skill_name in SUPABASE_SKILLS:
        ...
        if "~/.claude/scripts/supabase/" in content:
            count = content.count("~/.claude/scripts/supabase/")
            good_refs.append((skill_name, count))
            results.add_pass(f"{skill_name} references", f"Correctly uses ~/.claude/scripts/supabase/ ({count} times)")

        # Check for absolute /root references (potential bad)
        if "/root/.claude/scripts/supabase/" in content:
            bad_refs.append(skill_name)
            results.add_warning(f"{skill_name} paths", "Contains /root/.claude/scripts - should use ~/.claude/scripts/")

    if not bad_refs:
        results.add_pass("No absolute /root paths", "All skills use relative ~/.claude/scripts/ paths")
