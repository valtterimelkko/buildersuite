...
Location: `.claude/scripts/supabase/run-migrations.py`
...
5. **Run post-deployment audit:** `.claude/scripts/supabase/security-audit.sh` must pass (search_path on SECURITY DEFINER functions, RLS enabled) before proceeding
