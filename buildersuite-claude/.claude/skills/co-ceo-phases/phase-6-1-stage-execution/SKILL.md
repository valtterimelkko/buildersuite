...
description: Co-CEO Phase 6.1 - Orchestrate Implementation agents for each stage with hybrid complexity-aware git workflow. Simple stages stay on main, complex stages use worktrees with mandatory verification gates.
---
...
```bash
.claude/scripts/co-ceo/detect-stage-complexity.sh --verbose
```
...
```bash
# BEFORE Agent:
.claude/scripts/co-ceo/verify-stage-readiness.sh <stage-number>

# Include in agent prompt:
# "You are implementing Stage [N]: [Stage Name]"
# "Work directly on main branch. Commit frequently with conventional messages."
# "Do NOT create feature branches."

# AFTER Agent Completes:
.claude/scripts/co-ceo/verify-stage-completion.sh <stage-number>
...
# Include in agent prompt:
# "You are working on branch stage/[NN]-[stage-name]"
# "All commits go to this branch."
# "Do NOT merge to main—the Co-CEO will handle this after verification."

# AGENT COMPLETES...

# AFTER Agent Completes:
.claude/scripts/co-ceo/verify-stage-completion.sh <stage-number> stage/[NN]-[stage-name]
...
.claude/scripts/co-ceo/verify-stage-readiness.sh <stage-number> --strict
...
- You are working [on main branch | on branch stage/NN-name] ← Co-CEO specifies which
...
FOR EACH STAGE:
  ...
  4. SPAWN IMPLEMENTATION AGENT
     Launch with appropriate template + branch instruction
...
|--------|---------|-------|
| `detect-stage-complexity.sh` | Analyze stages and recommend strategy | `.claude/scripts/co-ceo/detect-stage-complexity.sh --verbose` |
| `verify-stage-readiness.sh` | Pre-stage verification gate | `.claude/scripts/co-ceo/verify-stage-readiness.sh <N> --strict` |
| `verify-stage-completion.sh` | Post-stage verification & merge confirm | `.claude/scripts/co-ceo/verify-stage-completion.sh <N> [branch]` |
| `git-commit-phase.sh` | Commit phase completion | `.claude/scripts/co-ceo/git-commit-phase.sh "6.1" "All stages complete"` |
...
for i in {1..N}; do
  .claude/scripts/co-ceo/verify-stage-completion.sh $i
done

# Commit final phase completion
.claude/scripts/co-ceo/git-commit-phase.sh "6.1" "All implementation stages complete"
