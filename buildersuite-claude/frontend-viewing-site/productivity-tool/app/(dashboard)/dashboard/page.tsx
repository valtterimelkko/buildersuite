...
export default async function DashboardPage() {
  ...
  const columns = {
    backlog: issues?.filter((i: { status: string }) => i.status === 'backlog') || [],
    todo: issues?.filter((i: { status: string }) => i.status === 'todo') || [],
    inprogress: issues?.filter((i: { status: string }) => i.status === 'inprogress') || [],
    done: issues?.filter((i: { status: string }) => i.status === 'done') || [],
  }
