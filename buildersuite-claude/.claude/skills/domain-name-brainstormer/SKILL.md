...
**Alternative (Claude Code Desktop only)**: If using Claude Code Desktop, you can set environment variables in your shell profile:
```bash
...
**Note for Claude Code Web/Mobile**: Claude Code Web and Mobile don't load shell profiles automatically, so the `.env` file approach is required.
...
Location: `.claude/scripts/check-domain.sh`

### Usage

```bash
# Check a single domain
.claude/scripts/check-domain.sh example.com

# Check multiple domains
.claude/scripts/check-domain.sh example.com mysite.io coolapp.dev

# Check one name across multiple TLDs
.claude/scripts/check-domain.sh myproject .com .io .dev .ai .co

# Pipe a list of domains
echo -e "domain1.com\ndomain2.io" | .claude/scripts/check-domain.sh
```
...
2. Run availability check:
   ```bash
   .claude/scripts/check-domain.sh snippetbox codeclip devpaste snipshare pastedev codebin sniphub clipcode .com .io .dev
   ```
...
./.claude/scripts/check-domain.sh mysite.com
```

**Using environment variables (Claude Code Desktop only):**
```bash
export PORKBUN_API_KEY="your-api-key"
export PORKBUN_API_SECRET="your-api-secret"
```
