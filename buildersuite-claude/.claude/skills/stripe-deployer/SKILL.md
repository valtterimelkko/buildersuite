...
- ✅ Use `stripe-webhook-checker` skill to validate handler completeness:
  ```bash
  python3 .claude/scripts/stripe/check_webhooks.py --file "path/to/webhook/route.ts"
  ```
...
Run verification:
```bash
python3 .claude/scripts/stripe/test_connection.py --test products
```
...
Location: `.claude/scripts/stripe/deploy-products.py`
