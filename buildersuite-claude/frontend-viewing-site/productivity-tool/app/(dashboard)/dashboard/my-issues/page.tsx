      ...
      <div className="flex-1 overflow-auto">
        {issues && issues.length > 0 ? (
          <div className="divide-y divide-border">
            {issues.map((issue: { id: string; title: string; status: string; priority: string; project?: { name: string; color: string }; labels?: Array<{ label: { id: string; name: string; color: string } }>; due_date?: string; identifier?: string }) => (
              <div
                key={issue.id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {issue.labels && issue.labels.length > 0 && (
                      <div className="flex items-center gap-1 mt-1">
                        {issue.labels.map((il: { label: { id: string; name: string; color: string } }) => (
                          <span
                            key={il.label.id}
                            className="text-xs px-1.5 py-0.5 rounded"
                            style={{
